...
import { loadPackagedWorker } from './packaged-worker.ts';
const worker = await loadPackagedWorker();

export async function measureRoute(route: string) {
  ...
  const root = resolve('dist/client'), seen = new Map<string, string>();
  function visit(url: string) {
    ...
    for (const node of tree.body) {
      if ((node.type !== 'ImportDeclaration' && node.type !== 'ExportNamedDeclaration' && node.type !== 'ExportAllDeclaration') || !node.source) continue;
