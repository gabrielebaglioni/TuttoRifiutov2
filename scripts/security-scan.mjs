...
function isDotenvPath(path) {
  return normalizePath(path).split("/").at(-1).startsWith(".env");
}

function scanMode(path) {
...
function unwrapExpression(node) {
  let value = node;
  while (value?.type === "ChainExpression" || value?.type === "ParenthesizedExpression") value = value.expression;
  return value;
}
...
function memberPropertyName(node) {
  ...
  if (!member.computed && ["Identifier", "PrivateIdentifier"].includes(member.property?.type)) {
...
function isProcessEnvMember(node) {
  ...
  return member?.type === "MemberExpression"
    ...
    && unwrapExpression(member.object)?.type === "Identifier"
    && unwrapExpression(member.object).name === "process";
}

function isRuntimePasswordRead(node) {
  const member = unwrapExpression(node);
...
function isPureRuntimeTemplate(node) {
  const value = unwrapExpression(node);
...
function isSafeRightHandSide(node) {
  const value = unwrapExpression(node);
...
function propertyHasPasswordKey(node) {
  if (!node?.key) return false;
  if (!node.computed && ["Identifier", "PrivateIdentifier"].includes(node.key.type)) {
    return node.key.name === PASSWORD_KEY;
  }
  return staticString(node.key) === PASSWORD_KEY;
}

function patternContainsSensitiveTarget(node, passwordContext = false) {
  const value = unwrapExpression(node);
...
function patternContainsEmbeddedPassword(node, passwordContext = false) {
  const value = unwrapExpression(node);
  if (!value) return false;
...
function isValueProperty(node, parent) {
...
function astContainsEmbeddedPassword(ast) {
  let embedded = false;
...
function javascriptContainsEmbeddedPassword(source, options) {
...
function dotenvValueIsEmbedded(rawValue) {
...
function dotenvContainsEmbeddedPassword(source) {
...
function extractAstroFragments(source) {
...
function astroContainsEmbeddedPassword(source) {
  const { fragments, malformed } = extractAstroFragments(source);
  if (malformed) return true;
  return fragments.some((fragment) => javascriptContainsEmbeddedPassword(fragment.source, fragment));
}

export function containsEmbeddedAdminPasswordAssignment(text, path = "inline.js") {
...
export function isProductionTrackedPath(path) {
...
export async function fileContainsBytes(path, value, options = {}) {
  ...
  for await (const chunk of createReadStream(path, { highWaterMark: requestedChunkSize })) {
...
export async function countFilesContainingBytes(paths, value) {
  let count = 0;
