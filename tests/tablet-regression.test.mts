...
test('touch menu initializes from the initial bundle and does not retry a denied GPU on tap',async()=>{
 let attempted=0;
 const c=vm.createContext({loadMenuLibrary:()=>({}),prefersReducedMotion:()=>false,showAtmosphereFallback(){},usesTouchLayout:()=>true,isOpen:false,atmosphereAttempted:false,atmosphereRenderer:null,initAtmosphere(){attempted++;throw new Error('GPU unavailable');}});
