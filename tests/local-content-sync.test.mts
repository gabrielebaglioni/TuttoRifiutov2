...
import { syncOnce, mediaPaths } from '../scripts/local-content-sync.mts';
...
import { d1Double, jsonRecord, record, sqlInput } from './worker-fixtures.mts';
...
test('snapshot fails closed on storage failure instead of exporting stale defaults', async () => {
  ...
  assert.equal((await publishedSnapshot({DB:d1Double({prepare(){throw new Error('private database detail');}})})).status,503);
});
...
test('real database export reflects published labels and excludes draft text and sessions', async () => {
  ...
  const DB = d1Double({ prepare(sql: string) { const stmt = db.prepare(sql); let args: SQLInputValue[] = []; return {
    bind(...values: unknown[]) { args = values.map(sqlInput); return this; }, async all() { return { results: stmt.all(...args) }; }
  }; } });
  ...
  const themed = await jsonRecord(await publishedSnapshot({DB}));
  record(themed.content);
...
test('sync rejects foreign or malformed formats before invoking a native decoder', async (t) => {
  ...
  for (const image of [Buffer.from('GIF89a foreign payload'), Buffer.from('RIFF0000WEBPmalformed')]) {
    ...
    await assert.rejects(syncOnce(root, async url => { assert.ok(typeof url === 'string'); return url.endsWith('.webp') ? new Response(image) : Response.json(snapshot); }));
