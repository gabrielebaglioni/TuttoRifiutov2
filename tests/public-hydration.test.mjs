...
test("content hydration updates only compatible marked fallback nodes", () => {
  ...
  assert.equal(document.querySelector("h1").textContent, "Titolo <sicuro>");
  assert.equal(document.querySelector("a").getAttribute("href"), "mailto:fallback@example.test");
  assert.equal(document.querySelector("p").textContent, "Sottotitolo");
});
...
test("variable editorial arrays replace their exact fallback structure through DOM builders", () => {
  ...
  assert.equal(document.querySelectorAll('[data-content-render="about"] h3').length, 3);
  assert.equal(document.querySelectorAll('[data-content-render="stats"] .stat-item').length, 1);
  assert.equal(document.querySelectorAll('[data-content-render="contact-rows"] .contact-info-row').length, 2);
  assert.equal(document.querySelectorAll('[data-content-render="project-meta"] p').length, 2);
  assert.equal(document.querySelectorAll('[data-content-render="project-columns"] .project-info-sub-col').length, 1);
});
...
test("collection renderers use DOM APIs and preserve rejected fallback media", () => {
  ...
  assert.equal(events.querySelector("a").getAttribute("href"), "/eventi/nuovo-evento");
  assert.equal(events.querySelector("p").textContent, "Evento <sicuro>");
  assert.equal(events.querySelector("img").getAttribute("src"), "/media/events/nuovo/00000000-0000-4000-8000-000000000002/640.webp");
  assert.equal(archive.textContent, "fallback", "unsafe data leaves the server fallback intact");
});
...
test("collection hydration retains local fallback media while updating a stored record's copy", () => {
  ...
  assert.equal(container.querySelector("img").getAttribute("src"), "/_astro/fallback.webp");
  assert.equal(container.querySelector(".work-item-info p").textContent, "Dopo");
});
...
test("archive fallback identity uses the stable slug even when cards share an href", () => {
  ...
  assert.equal(container.querySelector('[data-collection-slug="uno"] img').getAttribute("src"), "/one.webp");
  assert.equal(container.querySelector('[data-collection-slug="due"] img').getAttribute("src"), "/two.webp");
});
...
test("empty filters clear fallback cards and a new record without media receives a safe placeholder", () => {
  ...
  assert.equal(container.querySelector(".work-item-media-placeholder").getAttribute("aria-label"), "Nuovo");
});
...
test("detail hydration replaces event text, variable columns, and valid cover media", () => {
  ...
  assert.equal(root.querySelector("h1").textContent, "Aggiornato");
  assert.equal(root.querySelectorAll('[data-collection-render="meta"] p').length, 2);
  assert.equal(root.querySelectorAll('[data-collection-render="columns"] .project-info-sub-col').length, 1);
  assert.match(root.querySelector('[data-collection-media="cover"] img').getAttribute("src"), /^\/media\//);
});
...
test("project media-only hydration changes archival media without replacing published project copy", () => {
  ...
  assert.equal(root.querySelector("h1").textContent, "Titolo contenuto");
  assert.match(root.querySelector('[data-collection-media="cover"] img').getAttribute("src"), /^\/media\/archive\/parole\//);
});
...
test("worker rewrites SEO only for HTML assets and keeps static failures intact", async () => {
  ...
  const env = { DB, ASSETS: { fetch: async (request) => new Response(request.url.includes("/work") ? html : "plain", { status: request.url.includes("/work") ? 200 : 404, headers: { "content-type": request.url.includes("/work") ? "text/html" : "text/plain", "x-static": "kept" } }) } };
...
test("worker queries the route SEO and palette needed by initial HTML together", async () => {
  ...
  const DB = {
    prepare() {
      return {
        bind(...keys) { calls.push(keys); return this; },
...
test("home about renderer is attached to the about container, not the hero copy", () => {
  ...
  assert.doesNotMatch(hero[1], /data-content-render="about"/);
});
...
test("home hydration defaults leave the hero title and subtitle in their own nodes", () => {
  ...
  assert.equal(document.querySelector(".lab-hero h1").textContent, "Titolo pubblicato");
  assert.equal(document.querySelector(".lab-hero p").textContent, "Sottotitolo pubblicato");
  assert.equal(document.querySelectorAll(".lab-about h3").length, 2);
});
...
test("array defaults preserve the real about, stats, contact, and project markup exactly", () => {
  ...
  assert.deepEqual(nodes.map((node) => node.outerHTML), before);
});
...
test("array changes clone existing marked structures instead of discarding animation attributes", () => {
  ...
  const rows = document.querySelectorAll("h3");
  assert.equal(rows.length, 2);
  assert.equal(rows[0].getAttribute("data-animate-variant"), "slide");
  assert.equal(rows[1].getAttribute("data-animate-variant"), "slide");
  assert.equal(rows[1].querySelector("span").textContent, "Due");
});
...
test("event status changes reuse a fallback card from the other event panel", () => {
  ...
  const fallbacks = new Map([["spostato", upcoming.querySelector(".work-item-row")]]);
  ...
  assert.equal(past.querySelector("img").getAttribute("src"), "/_astro/esistente.webp");
  assert.equal(past.querySelector(".work-item-info p").textContent, "Dopo");
});
...
test("runtime responsive media uses distinct card, cover, and detail sizes", () => {
  ...
  const source = (id) => `/media/events/prova/00000000-0000-4000-8000-00000000000${id}/640.webp`;
  ...
  assert.equal(card.querySelector("img").getAttribute("sizes"), "(max-width: 1000px) 100vw, 40vw");
  assert.equal(detail.querySelector('[data-collection-media="cover"] img').getAttribute("sizes"), "100vw");
  assert.equal(detail.querySelector("[data-collection-media-list] img").getAttribute("sizes"), "(max-width: 1000px) 100vw, 75vw");
});
...
test("remote detail media restores its local fallback when the image fails", () => {
  ...
  const remote = root.querySelector('[data-collection-media="cover"] img');
  remote.dispatchEvent(new window.Event("error"));
  assert.equal(root.querySelector('[data-collection-media="cover"] img').getAttribute("src"), "/_astro/local.webp");
});
...
test("remote card media restores its local fallback and uses a placeholder for a new record", () => {
  ...
  existing.querySelector("img").dispatchEvent(new window.Event("error"));
  assert.equal(existing.querySelector("img").getAttribute("src"), "/_astro/card.webp");
  ...
  fresh.querySelector("img").dispatchEvent(new window.Event("error"));
  assert.equal(fresh.querySelector(".work-item-media-placeholder").getAttribute("aria-label"), "Nuovo");
});
