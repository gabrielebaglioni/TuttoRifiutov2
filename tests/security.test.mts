...
import * as securityScan from "../scripts/security-scan.mts";

const { containsEmbeddedAdminPasswordAssignment } = securityScan;

test('Astro TypeScript declarations are inspected without rejecting safe typed runtime reads', () => {
  assert.equal(containsEmbeddedAdminPasswordAssignment('---\ninterface Props { title: string }\nconst password: string = env.ADMIN_PASSWORD;\nreturn;\n---\n<p>Safe</p>', 'Typed.astro'), false);
  assert.equal(containsEmbeddedAdminPasswordAssignment('---\nconst ADMIN_PASSWORD: string = "fixture-value";\n---', 'Typed.astro'), true);
});

function detect(source: string, path = "inline.js") {
  return containsEmbeddedAdminPasswordAssignment(source, path);
}

const TEXT_EXTENSIONS = new Set([
  "", ".astro", ".cjs", ".css", ".env", ".html", ".js", ".json",
  ".map", ".md", ".mjs", ".mts", ".sql", ".svg", ".toml", ".ts", ".tsx",
  ".txt", ".xml", ".yaml", ".yml",
]);
...
test("production scan excludes test and documentation fixtures but includes shipped sources", () => {
  ...
  assert.equal(securityScan.isProductionTrackedPath("tests/auth.test.mts"), false);
  ...
  assert.equal(securityScan.isProductionTrackedPath("scripts/build-worker.mts"), true);
...
function trackedFiles() {
  return new TextDecoder().decode(execFileSync("git", ["ls-files", "-z"]))
    .split("\0")
    .filter(Boolean);
}
