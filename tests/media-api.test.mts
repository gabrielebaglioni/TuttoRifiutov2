...
import { d1Double, d1Statement, r2Double, required, sqlNumber, sqlString, jsonRecord, recordRows } from './worker-fixtures.mts';
import type { MediaStore } from '../worker/types.ts';
import type { D1PreparedStatement, R2HTTPMetadata, R2PutOptions } from '@cloudflare/workers-types/index.ts';
type MediaFixture = { id: number; event_id?: number; archive_item_id?: number; key: string; role: string; alt: string; position: number; widths_json: string; sources_json?: string; state?: string; reservation_started_at?: number; created_at?: number; cleanup_attempts?: number };
type ParentFixture = { id: number; slug: string; status?: string; deleting?: number };
type DbOptions = { media?: MediaFixture[]; archiveMedia?: MediaFixture[]; failInsert?: boolean; enforceSlotUnique?: boolean; synchronizeInitialSlotReads?: boolean; activationChanges?: number; parents?: ParentFixture[] };
type R2Options = { failPutAt?: number; failDeleteAt?: number };
type FormFields = { ownerType?: string; ownerSlug?: string; role?: string; position?: number; alt?: string; small?: File | null; medium?: File | null; large?: File | null };
...
function extendedWebp(chunks: number[][]) {
  const body = [0x57, 0x45, 0x42, 0x50, ...chunks.flat()];
  return new Uint8Array([0x52, 0x49, 0x46, 0x46, ...le32(body.length), ...body]);
}

function image(width: number, { type = "image/webp", bytes = webp(width, Math.max(1, Math.floor(width * 0.6))) } = {}) {
...
function fixtureFile(base64: string, name = "fixture.webp") {
  return new File([Buffer.from(base64, "base64")], name, { type: "image/webp" });
}

function corrupt(file: File, changes: [number, number][]) {
...
function form(fields: FormFields = {}) {
  ...
  for (const [field, width] of [["small", 640], ["medium", 1280], ["large", 2048]] as const) {
    if (fields[field] !== null) body.set(field, fields[field] ?? image(width));
  }
  return body;
}

function makeMedia(key = "events/musica/00000000-0000-4000-8000-000000000001/2048.webp"): MediaFixture & { role: 'cover' } {
  return { id: 1, event_id: 1, key, role: "cover", alt: "Vecchia copertina", position: 0, widths_json: "[640,1280,2048]" };
}

function deliveryEnvironment(row: MediaFixture, { dbFailure = false, objectFailure = false, object = true, parentSlug = row.key.split("/")[1] } = {}) {
  return {
    DB: d1Double({
      prepare(sql: string) {
...
function createDb({ media = [], archiveMedia = [], failInsert = false, enforceSlotUnique = false, synchronizeInitialSlotReads = false, activationChanges, parents = [{ id: 1, slug: "musica" }] }: DbOptions = {}) {
  const rows: { events: ParentFixture[]; archive: ParentFixture[] } = { events: parents.map((parent) => ({ status: 'published', deleting: 0, ...parent })), archive: [] };
  ...
  let slotReads = 0;
  let releaseSlotReads: (() => void) | undefined;
  const slotReadsReady = new Promise<void>((resolve) => { releaseSlotReads = resolve; });
  const db = {
        ...
        bind: (...args: unknown[]) => ({
          async first() {
            if (sql.includes("JOIN")) {
              ...
              return media ? { ...media, owner_slug: required(owner).slug } : null;
            }
            ...
            if (sql.includes("SELECT * FROM event_media") || sql.includes("SELECT * FROM archive_media")) {
              ...
              if (sql.includes("key = ?")) return storedMedia[kind].find((entry) => entry.state === "active" && (entry.key === args[0] || String(entry.sources_json ?? "").includes(sqlString(args[0])))) ?? null;
              slotReads += 1;
              if (synchronizeInitialSlotReads && slotReads <= 2) {
                if (slotReads === 2) required(releaseSlotReads)();
          ...
          async all() {
            ...
            if (sql.includes("reservation_started_at")) {
              const cutoff = sqlNumber(args[0]);
              return { results: storedMedia[kind].filter((entry) => entry.state === "tombstone" || entry.state === "pending_cleanup" || (entry.state === "pending" && (entry.reservation_started_at ?? 0) > 0 && (entry.reservation_started_at ?? 0) <= cutoff)) };
            }
            if (sql.includes("tombstone") || sql.includes("pending_cleanup")) return { results: storedMedia[kind].filter((entry) => entry.state === "tombstone" || entry.state === "pending_cleanup") };
            return { results: [] };
          },
          async run() {
            ...
            if (sql.startsWith("INSERT INTO events") || sql.startsWith("INSERT INTO archive_items")) {
              const inserted = { id: rows[kind].length + 1, slug: sqlString(args[0]) };
            ...
            if (sql.startsWith("INSERT INTO event_media") || sql.startsWith("INSERT INTO archive_media")) {
              ...
              const parentId = sqlNumber(args.at(-1));
              ...
              storedMedia[kind].push({ id: nextMediaId++, [parent]: parentId, key: sqlString(args[0]), role: sqlString(args[1]), alt: sqlString(args[2]), position: sqlNumber(args[3]), widths_json: sqlString(args[4]), sources_json: sqlString(args[5]), state: "pending", reservation_started_at: sqlNumber(args[6]), created_at: sqlNumber(args[7]), cleanup_attempts: 0 });
            ...
            if (sql.startsWith("UPDATE event_media") || sql.startsWith("UPDATE archive_media")) {
              ...
              if (sql.includes("SET state = 'pending_cleanup'")) {
                const pending = storedMedia[kind].find((entry) => entry.id === args[0] && entry.state === "pending" && (!sql.includes("reservation_started_at <=") || ((entry.reservation_started_at ?? 0) > 0 && (entry.reservation_started_at ?? 0) <= sqlNumber(args[1]))));
                if (pending) {
    ...
    async batch(items: D1PreparedStatement[]) { return Promise.all(items.map((item) => item.run())); },
  };
  return d1Double(db);
}

function createR2({ failPutAt, failDeleteAt }: R2Options = {}) {
  const objects = new Map<string, { value: Blob; options: { httpMetadata: R2HTTPMetadata } }>();
  ...
  return r2Double({
    ...
    async put(key: string, value: Parameters<MediaStore['put']>[1], options?: R2PutOptions) {
      ...
      objects.set(key, { value, options: { httpMetadata } });
    },
    async delete(key: string | string[]) { assert.ok(typeof key === 'string'); deleted.push(key); deletes += 1; if (deletes === failDeleteAt) throw new Error("R2 delete failed"); objects.delete(key); },
    async get(key: string) {
      const entry = objects.get(key);
      return entry && {
        body: entry.value.stream(), httpEtag: '"fixture-etag"',
        writeHttpMetadata(headers: Headers) { headers.set("content-type", required(entry.options.httpMetadata.contentType)); },
      };
    },
  });
}

async function environment(options: DbOptions & R2Options & { DB?: ReturnType<typeof createDb>; MEDIA?: ReturnType<typeof createR2> } = {}) {
  ...
  DB.prepare = (sql) => {
    if (sql.includes("FROM sessions")) return d1Statement({ bind: () => ({ first: async () => ({ token_hash: "hash", csrf_token: csrfToken, expires_at: Date.now() + 60_000 }) }) });
...
function uploadRequest(env: { cookie: string; csrfToken: string }, body = form()) {
...
test("only structurally valid RIFF VP8 VP8L and VP8X WebPs pass after independent decoding", async () => {
  ...
  for (const [index, fixture] of fixtures.entries()) {
    ...
    assert.deepEqual(await inspectWebp(fixture), {
      width: required(DECODED_WEBP_FIXTURES[index]).width,
      height: required(DECODED_WEBP_FIXTURES[index]).height,
    });
  ...
  const malformed = [
    await corrupt(required(fixtures[0]), [[20, 1]]), // VP8 inter-frame tag, not a still-image key frame.
    await corrupt(required(fixtures[1]), [[24, 0xe0]]), // VP8L reserved version bits.
    await corrupt(required(fixtures[2]), [[21, 1]]), // VP8X reserved bytes must be zero.
    await corrupt(required(fixtures[2]), [[24, 2]]), // VP8X canvas must agree with the encoded image.
    await corrupt(required(fixtures[2]), [[46, 0x41], [47, 0x4e], [48, 0x4d], [49, 0x46]]), // ANMF cannot stand in for image data.
    await corrupt(required(fixtures[0]), [[4, 0]]), // Incorrect RIFF length.
  ];
...
test("upload requires an authenticated CSRF request and accepts one to three correctly ordered optimized variants", async () => {
  ...
  assert.ok(required([...env.MEDIA.objects.keys()][0]).endsWith("/500.webp"));
});
...
test("upload preserves actual no-upscale widths and rejects field-order or pixel-limit violations", async () => {
  ...
  assert.deepEqual((await jsonRecord(accepted)).widths, [640, 1000]);
...
test("a relevant public fetch schedules retained tombstone retry through the production waitUntil context", async () => {
  ...
  assert.equal(required(env.DB.storedMedia.events[0]).cleanup_attempts, 2);
...
test("media mutations fail with 500 when either required storage binding is absent", async () => {
  const { MEDIA: _removedMedia, ...missingMedia } = await environment();
  assert.equal((await routeRequest(uploadRequest(missingMedia), missingMedia, {})).status, 500);
  const { DB: _removedDb, ...missingDb } = await environment();
...
test("every R2 put and delete stage compensates or leaves a non-public retryable tombstone", async () => {
  ...
  for (const failDeleteAt of [1, 2, 3]) {
    ...
    const response = await routeRequest(new Request("https://site.test/api/admin/media/events/1", { method: "DELETE", headers: { cookie: env.cookie, "x-csrf-token": env.csrfToken } }), env, {});
    assert.equal(response.status, 202);
    assert.equal(required(env.DB.storedMedia.events[0]).state, "tombstone");
    assert.equal(await retryTombstones(env, "events"), 1);
    assert.equal(env.DB.storedMedia.events.length, 0);
  }
});
...
test("a rollback delete failure stays tracked as pending cleanup until a retry can remove it", async () => {
  ...
  const scheduled: Promise<unknown>[] = [];
  const response = await routeRequest(uploadRequest(env), env, { waitUntil(promise: Promise<unknown>) { scheduled.push(promise); } });
  assert.equal(response.status, 500);
  await Promise.all(scheduled);
...
test("upload materializes a fallback parent and stores only immutable optimized WebP variants", async () => {
  ...
  assert.ok(Array.isArray(body.widths));
  assert.equal(body.widths.join(","), "640,1280,2048");
  assert.deepEqual(recordRows(body.sources).map((source) => source.width), widths);
  assert.ok(recordRows(body.sources).every((source) => /^\/media\/events\/musica\/[0-9a-f-]+\/(640|1280|2048)\.webp$/.test(sqlString(source.src))));
...
test("replacement commits the new reference before deferred cleanup of every old variant", async () => {
  ...
  const scheduled: Promise<unknown>[] = [];
  const ctx = { waitUntil(promise: Promise<unknown>) { scheduled.push(promise); } };
  const response = await routeRequest(uploadRequest(env), env, ctx);
  assert.equal(response.status, 201);
  assert.equal(env.DB.storedMedia.events.filter((entry) => entry.state === "active").length, 1);
  assert.notEqual(required(env.DB.storedMedia.events.find((entry) => entry.state === "active")).key, old.key);
  assert.equal(scheduled.length, 1);
  await Promise.all(scheduled);
  assert.equal(env.DB.storedMedia.events.length, 1);
  assert.equal(required(env.DB.storedMedia.events[0]).id !== old.id, true);
