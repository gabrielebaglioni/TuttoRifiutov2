...
test('splatter alpha stays transparent outside the silhouette and covers the viewport after zoom', async () => {
  ...
  for (const [x, y] of [[367, 64], [735, 351], [73, 727], [528, 721]] as const) {
    assert.ok(alpha(x, y) >= 250, `droplet at ${x},${y} preserved`);
  }
  for (const [width, height] of [[320, 740], [390, 844], [360, 900], [844, 390], [768, 1024], [1920, 1080], [3440, 1440], [5120, 1440]] as const) {
