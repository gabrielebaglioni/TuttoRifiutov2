...
test("fallback media expose browser-usable URLs and stable source identifiers", () => {
  assert.equal(EVENT_DEFAULTS[0].cardMedia.src, "/fallback/eventi/bruciaRifiuti.webp");
  assert.equal(EVENT_DEFAULTS[0].cardMedia.fallbackAsset, "src/assets/optimized/eventi/bruciaRifiuti.webp");
  assert.equal(ARCHIVE_DEFAULTS[0].coverMedia.src, "/fallback/work/work_01.webp");
});

function createDb() {
  const rows = { events: [], archive: [] };
  const media = { events: [], archive: [] };
  let nextId = 1;
...
async function environmentWithSession() {
  ...
  return {
    DB,
    MEDIA: { delete: async () => {}, get: async () => null },
    SESSION_SECRET,
    csrfToken,
...
test("public collection reads retain complete default records and append stored records in stable order", async () => {
  ...
  for (const field of ["meta", "info", "outroInfo", "seo"]) assert.deepEqual(createdBody[field], eventPayload[field], field);
  assert.equal(env.DB.batchCalls, 1);
  const response = await routeRequest(new Request("https://site.test/api/events"), env, {});
  const events = await response.json();
  assert.equal(response.status, 200);
  assert.deepEqual(events.map((event) => event.position), [...events].map((event) => event.position).sort((left, right) => left - right));
  ...
  assert.equal((await (await routeRequest(new Request("https://site.test/api/events/nuovo-evento"), env, {})).json()).title, "Nuovo evento");
});
...
test("draft events remain editable API values but are excluded from public list and slug reads", async () => {
  ...
  assert.equal(created.status, 201);
  assert.equal((await (await routeRequest(new Request("https://site.test/api/events"), env, {})).json()).some((item) => item.slug === draft.slug), false);
...
test("authenticated admin collection reads include drafts, hide deleting rows, and never cache", async () => {
  ...
  env.DB.rows.events.push({ ...env.DB.rows.events[0], id: 99, slug: "in-eliminazione", deleting: 1 });
...
test("collection reorder validates and returns the complete non-deleting admin set including drafts", async () => {
  ...
  const admin = await (await routeRequest(new Request("https://site.test/api/admin/events", { headers: { cookie: env.cookie } }), env, {})).json();
  ...
  assert.equal((await reordered.json()).some((item) => item.slug === draft.slug && item.status === "draft"), true);
});
...
test("stored media are joined, normalized, ordered, and exposed only through media", async () => {
  ...
  const item = await (await routeRequest(new Request("https://site.test/api/events/nuovo-evento"), env, {})).json();
  assert.deepEqual(item.media.map((entry) => entry.id), [7, 8]);
  assert.equal(item.cardMedia, undefined);
  assert.equal(item.heroMedia, undefined);
  assert.equal(item.detailMedia, undefined);
  assert.deepEqual(item.media[0].widths, [640, 1280, 2048]);
  assert.deepEqual(item.media[0].sources, [
...
test("stored media never overwrite an untouched default's fallback media fields", async () => {
  ...
  const item = await (await routeRequest(new Request("https://site.test/api/events/giornata-tutto-rifiuto"), env, {})).json();
  ...
  assert.deepEqual(item.media.map(({ id, role, position }) => ({ id, role, position })), [
  ...
  const archive = await (await routeRequest(new Request("https://site.test/api/archive/parole"), env, {})).json();
  ...
  assert.deepEqual(archive.media.map(({ id, role, position }) => ({ id, role, position })), [{ id: 73, role: "cover", position: 0 }]);
});
...
test("row and media position changes execute in one batch", async () => {
  ...
  assert.equal(env.DB.media.events[0].position, 3);
...
test("admin collection mutations reject unauthenticated, invalid, and duplicate records", async () => {
  const env = await environmentWithSession();
  const request = (body, headers = {}) => new Request("https://site.test/api/admin/events", { method: "POST", headers: { "content-type": "application/json", ...headers }, body: JSON.stringify(body) });
...
test("archive updates preserve exact fields and deletes remove only the override", async () => {
  ...
  const archive = await (await routeRequest(new Request("https://site.test/api/archive/nuovo-archivio"), env, {})).json();
  assert.deepEqual(Object.fromEntries(["title", "code", "href", "description", "details", "outro", "seo", "position"].map((key) => [key, archive[key]])), Object.fromEntries(["title", "code", "href", "description", "details", "outro", "seo", "position"].map((key) => [key, changed[key]])));
...
test("updating and deleting a fallback event creates and restores only its override", async () => {
  const env = await environmentWithSession();
  const headers = { cookie: env.cookie, "content-type": "application/json", "x-csrf-token": env.csrfToken };
  const fallback = await (await routeRequest(new Request("https://site.test/api/events/musica"), env, {})).json();
  ...
  assert.equal((await (await routeRequest(new Request("https://site.test/api/events/musica"), env, {})).json()).title, "Musica aggiornata");
...
test("a cleanup-pending deletion is immediately absent from public collection routes but remains retryable", async () => {
  ...
  const list = await (await routeRequest(new Request("https://site.test/api/events"), env, {})).json();
