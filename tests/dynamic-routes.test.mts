...
import { d1Double, d1Statement, required, record, sqlString } from "./worker-fixtures.mts";
import test from "node:test";
import { parseHTML } from "linkedom";
import { routeRequest, templatePathFor } from "../worker/router.ts";
import { detailIdentityFromPath, hydrateDetailTemplate, renderArchive } from "../src/scripts/collections-hydration.ts";
...
import { adminSectionForContentKey } from "../src/scripts/admin.ts";
...
function dynamicDb(row: unknown, { fail = false, failContent = false } = {}) {
...
function assets({ failOriginal = false, failTemplate = false } = {}) {
  const calls: { pathname: string; method: string; headers: Headers }[] = [];
  return {
    calls,
  ...
  for (const { path, row, status } of cases) {
    for (const method of ["GET", "HEAD"]) {
      ...
      if (status === 404) {
        ...
        if (method === "GET" && row) {
          const seo: unknown = JSON.parse(row.seo_json); record(seo);
          assert.match(await response.text(), new RegExp(sqlString(seo.title)));
        }
        else if (method === "GET") {
...
test("details without a stored collection override still fetch full fresh representations for palette injection", async () => {
  ...
  assert.equal(required(calls[0]).get("if-none-match"), null);
  assert.equal(required(calls[0]).get("if-modified-since"), null);
  ...
  assert.equal(required(calls[1]).get("range"), null);
  assert.equal(required(calls[1]).get("if-range"), null);
});
...
test("a public stored override sanitizes validators and returns fresh no-store GET and HEAD representations", async () => {
  ...
  for (const method of ["GET", "HEAD"]) {
    ...
    assert.match(required(response.headers.get("content-security-policy")), /frame-ancestors 'none'/);
...
test("archive dynamic details use the archive template while project stays a static authority", async () => {
  ...
  const projectAssets = { calls: projectPaths, async fetch(input: RequestInfo | URL) { const request = new Request(input); this.calls.push(new URL(request.url).pathname); return new Response("project static", { headers: { "content-type": "text/html" } }); } };
...
test("dynamic misses are no-store bodyless HEAD responses and do not hide a later record", async () => {
  let row: typeof publishedEvent | null = null;
  const DB = dynamicDb(null);
  DB.prepare = (sql) => d1Statement({
    bind: (...values: unknown[]) => ({
...
test("the shared hydrator fetches the safe event slug and updates generic fallback with DOM APIs", async () => {
  ...
  assert.equal(required(root.querySelector("h1")).textContent, "Titolo <sicuro>");
  assert.equal(root.querySelector("h1 img"), null);
  assert.match(required(required(root.querySelector('[data-collection-media="cover"] img')).getAttribute("src")), /^\/media\/events\//);
});
...
test("template hydration rejects mismatched or unsafe paths without fetching or replacing fallback", async () => {
  for (const pathname of ["/archivio/nuovo", "/eventi/template", "/eventi/a%2Fb"]) {
    ...
    assert.equal(required(document.querySelector("h1")).textContent, "Fallback accessibile", pathname);
  }
});
...
test("new archive records link to their detail URL while existing project links remain unchanged", () => {
  ...
  const container = required(document.querySelector<HTMLElement>("#archive"));
  ...
  assert.equal(required(container.querySelector('[data-collection-slug="parole"] a')).getAttribute("href"), "/project");
  assert.equal(required(container.querySelector('[data-collection-slug="nuovo-pacchetto"] a')).getAttribute("href"), "/archivio/nuovo-pacchetto");
});
...
test("archive template labels are complete editable defaults in public, worker, and admin schemas", () => {
  ...
  for (const [key, value] of Object.entries(expected)) {
    assert.equal(Reflect.get(SITE_CONTENT, key), value, key);
    assert.equal(Reflect.get(WORKER_CONTENT, key), value, key);
