...
import type { ThemeUniforms } from '../src/scripts/theme.ts';
...
test('initial public HTML contains stored theme even before client content hydration', async () => {
  const env = { DB: d1Double({ prepare() { return { bind() { return this; }, all: async () => ({results:[{key:'global.theme.palette', value_json:JSON.stringify({...DEFAULT_PALETTE, accent:'#112233'})}]}) }; } }), ASSETS:{fetch:async()=>new Response('<html><head><title>Old</title></head><body></body></html>', {headers:{'content-type':'text/html'}})} };
...
for (const path of ['/eventi/musica', '/archivio/parole']) test(`static detail ${path} receives theme without a collection override and preserves static SEO`, async () => {
  ...
  const response=await routeRequest(new Request(`https://site.test${path}`),{DB: d1Double(DB),ASSETS:{fetch:async()=>new Response(html,{headers:{'content-type':'text/html',etag:'compiled'}})}},{});
...
test('grain uniforms follow palette changes and redraw subscribers without frame-time style reads', async () => {
  ...
  Reflect.set(must(document.defaultView), "getComputedStyle", (element: HTMLElement) => element.style);
  const uniforms: ThemeUniforms = {}; let redraws=0;
  ...
  assert.deepEqual(must(uniforms.uColorBg).value,[1,0,0]); assert.deepEqual(must(uniforms.uColorFg).value,[0,0,1]); assert.equal(redraws,2);
...
test('mobile pie recolors cached masks and repaints without reallocating geometry', async () => {
  ...
  const saved = Object.fromEntries(['document','window','Image','requestAnimationFrame','cancelAnimationFrame'].map(key=>[key,Reflect.get(globalThis, key)]));
  const contexts: Array<{calls: Array<{color: string; composite: string}>}> = []; const callbacks: FrameRequestCallback[] = []; let sourceImage: ImageDouble | undefined;
  class ImageDouble { naturalWidth = 800; naturalHeight = 800; onload?: () => void; constructor() { sourceImage = this; } }
  const context = () => {const calls: Array<{color: string; composite: string}>=[]; const c={calls,fillStyle:'',globalCompositeOperation:'',setTransform(){},clearRect(){},translate(){},scale(){},drawImage(){},save(){},restore(){},beginPath(){},moveTo(){},arc(){},closePath(){},clip(){},fill(){},fillRect(){calls.push({color:this.fillStyle,composite:this.globalCompositeOperation});}}; contexts.push(c);return c;};
  try {
    installGlobal("document", {createElement(){const c=context();return {width:0,height:0,setAttribute(){},getContext:()=>c,remove(){}};}});
    installGlobal("window", {devicePixelRatio:2}); installGlobal("Image", ImageDouble);
    ...
    const { document: fixtureDocument } = parseHTML('<main></main>');
    const container = must(fixtureDocument.querySelector('main'));
    Object.assign(container, { clientWidth: 390, clientHeight: 800, appendChild() {} });
    const canvas=createPieCanvas(container,{imageUrl:'/mask.webp',origin:{x:400,y:400},box:{x:0,y:0,width:800,height:800},color:'#2444d9',onError(){assert.fail('unexpected canvas failure');}});
    must(must(sourceImage).onload)(); must(callbacks.shift())(0); const allocations=contexts.length;
    must(canvas).draw(.8,7); must(canvas).setColor('#123456'); assert.equal(contexts.length,allocations);
    assert.deepEqual(contexts.slice(-2).map(c=>c.calls.at(-1)),[{color:'#123456',composite:'source-in'},{color:'#123456',composite:'source-in'}]);
    assert.equal(callbacks.length,1);must(canvas).destroy();
  } finally {for(const [key,value] of Object.entries(saved)){restoreGlobal(key, value);}}
});
...
test('palette accepts complete hex colors and rejects incomplete, injected and exotic objects', () => {
  ...
  const incomplete = {...DEFAULT_PALETTE}; Reflect.deleteProperty(incomplete, "foreground");
