...
test("explicit index.html aliases cannot bypass runtime SEO or collection publication gates", async () => {
  const worker = await loadCompiledWorker(new URL(`../dist/server/index.js?gate=${Date.now()}`, import.meta.url));
  const env = {
...
for (const path of ['/', '/eventi/giornata-tutto-rifiuto', '/eventi/nuovo-evento']) test(`compiled ${path} metadata uses the current public Sites origin`, async () => {
  ...
  for (const [selector,attribute] of [['link[rel="canonical"]','href'],['meta[property="og:url"]','content'],['meta[property="og:image"]','content'],['meta[name="twitter:image"]','content']] as const) {
