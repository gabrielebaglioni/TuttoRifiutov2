...
import { d1Double, required, sqlNumber, sqlString, jsonRecord } from './worker-fixtures.mts';

function createDb() {
  const sessions = new Map<unknown, { token_hash: string; csrf_token: string; expires_at: number; created_at: number }>();
  const attempts: { id: number; ip_hash: string; attempted_at: number }[] = [];
  let nextAttemptId = 1;
        ...
        bind: (...args: unknown[]) => ({
          async run() {
            if (sql.includes("INSERT INTO sessions")) {
              sessions.set(args[0], {
                token_hash: sqlString(args[0]),
                csrf_token: sqlString(args[1]),
                expires_at: sqlNumber(args[2]),
                created_at: sqlNumber(args[3]),
              });
              ...
              const kept = attempts.filter((attempt) => attempt.attempted_at >= sqlNumber(args[0]));
              ...
              attempts.push({ id: nextAttemptId++, ip_hash: sqlString(args[0]), attempted_at: sqlNumber(args[1]) });
            }
            return { success: true };
          },
          async first() {
            ...
            if (sql.includes("INSERT INTO login_attempts")) {
              ...
              const count = attempts.filter((attempt) => attempt.ip_hash === countedIpHash && attempt.attempted_at >= sqlNumber(since)).length;
              if (count >= sqlNumber(limit)) return null;
              const attempt = { id: nextAttemptId++, ip_hash: sqlString(ipHash), attempted_at: sqlNumber(attemptedAt) };
              attempts.push(attempt);
              return { id: attempt.id };
            }
            if (sql.includes("COUNT(*) AS count")) {
              return { count: attempts.filter((attempt) => attempt.ip_hash === args[0] && attempt.attempted_at >= sqlNumber(args[1])).length };
            }
            return null;
          },
        }),
...
function environment() {
  return {
    ...
    ADMIN_PASSWORD: String(globalThis.crypto.randomUUID()),
...
function loginRequest(username: string, password: string, ip = "203.0.113.10") {
...
function cookieFrom(response: Response): string {
  return required(required(response.headers.get("set-cookie")).split(";")[0]);
}
...
test("login creates a hashed eight-hour server session and strict cookie", async () => {
  ...
  assert.equal(sqlString(body.csrfToken).length, 64);
  const cookie = required(response.headers.get("set-cookie"));
  ...
  const stored = required([...env.DB.sessions.values()][0]);
...
test("failed and throttled logins return the same generic response", async () => {
  ...
  assert.equal(required(env.DB.attempts[0]).ip_hash.includes("203.0.113.10"), false);
});
...
test("session access and writing CSRF checks are enforced", async () => {
  ...
  const csrfToken = sqlString((await jsonRecord(loginResponse)).csrfToken);
  ...
  assert.ok(missingCsrf instanceof Response);
