...
import { d1Double, required, sqlString, jsonRecord } from './worker-fixtures.mts';
...
test('palette content writes reject invalid and CSRF-less values before persistence and save one complete object', async () => {
  ...
  const put = (value: unknown, csrf=true) => routeRequest(new Request(`https://site.test/api/admin/content/${THEME_KEY}`, {
  ...
  assert.deepEqual(JSON.parse(required(env.DB.content.get(THEME_KEY))),next);
  const reload=await routeRequest(new Request('https://site.test/api/content'),env,{});
  assert.deepEqual((await jsonRecord(reload))[THEME_KEY],next);
});
...
test("stored overrides replace only matching default keys", () => {
  ...
  const result = mergeContent(defaults, [{ key: "home.hero.title", value_json: '"Altro titolo"', published: 1, updated_at: 0 }]);
...
test("malformed, unknown, and wrong-type stored overrides leave the fallback intact", () => {
  ...
  const result = mergeContent(defaults, [
    { key: "missing.key", value_json: '"Non visibile"', published: 1, updated_at: 0 },
    { key: "home.hero.title", value_json: "{", published: 1, updated_at: 0 },
    { key: "home.hero.title", value_json: "[\"Non una stringa\"]", published: 1, updated_at: 0 },
  ]);
...
test("array overrides match list element schemas and nested tuple arity", () => {
  ...
  assert.deepEqual(mergeContent(defaults, [
    { key: "contact.info.rows", value_json: '[["Etichetta modificata"]]', published: 1, updated_at: 0 },
  ]), defaults);
});
...
function createDb() {
  const content = new Map<unknown, string>();
    ...
    prepare(sql: string) {
      return {
        ...
        bind: (...args: unknown[]) => ({
          ...
          async run() {
            if (sql.includes("INSERT INTO content_entries")) content.set(args[0], sqlString(args[1]));
...
test("public content reads merge published overrides with fallbacks", async () => {
  ...
  const content = await jsonRecord(response);
...
test("valid writes merge into public content and delete restores the fallback", async () => {
  ...
  assert.equal((await jsonRecord(published))["home.hero.title"], "Titolo pubblicato");
...
test("content writes reject unknown keys, invalid types, oversize strings, and unsafe links", async () => {
  ...
  const invalidWrites: [string, unknown][] = [
    ["missing.key", "No"],
