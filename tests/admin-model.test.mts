...
import { AdminDrafts, AuthEpoch, BusyResources, DirtyResources, buildCollectionReorderRequest, buildContentRequest, buildMediaMetadataRequest, buildMediaOrderRequest, collectionSlugControl, mediaPreviewKind, mergeMediaSlots, runBusyResource, setAtPath, updateDraft } from "../src/scripts/admin-model.ts";
...
test("dirty resources clear only the saved resource", () => {
    ...
    dirty.mark("content:a");
    dirty.mark("content:b");
    dirty.clear("content:a");
    assert.equal(dirty.count, 1);
    assert.equal(dirty.has("content:b"), true);
});
...
test("busy resources prevent duplicate operations until released", () => {
    ...
    assert.equal(busy.acquire("upload:1"), null);
...
test("central drafts retain collection, unsaved, and media changes without replacing an input or sibling model", () => {
    ...
    assert.equal(readChild(drafts.begin(resource, { title: "Da non sovrascrivere" }), "title"), "Titolo aggiornato");
});
test("slot-aware media merge retains virtual fallbacks until an active stored slot replaces them", () => {
    ...
    assert.equal(required(revealed[2]).src, "/fallback-detail.webp");
    assert.equal(required(revealed[2]).virtual, true);
});
...
test("draft rebase preserves unsaved editorial paths while refreshing named server-owned paths", () => {
    ...
    assert.equal(readChild(drafts.get(resource), "title"), "Titolo non salvato");
    assert.deepEqual(readChild(drafts.get(resource), "media"), [{ id: 2, role: "cover", position: 0, src: "/media/new.webp" }]);
    assert.deepEqual(readChild(drafts.get(resource), "detailMedia"), [{ src: "/fallback/new-detail.webp" }]);
    assert.equal(readChild(drafts.get(resource), "position"), 2);
});
test("draft revisions and resource prefixes prevent stale cleanup and preserve child identity transitions", () => {
    ...
    assert.equal(readChild(drafts.get(newParent), "title"), "Edit successivo");
    ...
    assert.equal(readChild(drafts.get("collection:events:41"), "title"), "Edit successivo");
