...
type ProjectWindow = Window & { CustomEvent: typeof CustomEvent };
export type ProjectOutcome = { started: true; status: "started" } | { started: false; status: "unsupported" | "error" };
export type ProjectStartDetail = Readonly<{ count: number; images: readonly string[] } & ProjectOutcome>;
...
export class ProjectDistortion {
  ...
  createMeshes() {
    ...
    const media = [...document.querySelectorAll<HTMLImageElement>(".project-img img")];
    Promise.all(media.map((image) => loadGalleryImage(image, () => window.scrollY || window.pageYOffset || 0))).then((loadedImages) => {
      this.mediaStore = loadedImages.filter((bounds) => bounds !== null).map((bounds) => {
  ...
  showNativeImages() {
    this.failed = true;
    if (this.renderer) this.renderer.domElement.style.display = "none";
    document.querySelectorAll<HTMLImageElement>(".project-img img").forEach((image) => { image.style.opacity = "1"; });
  }
...
function projectImageSnapshot(documentRef: Document) {
  return Object.freeze([...documentRef.querySelectorAll<HTMLImageElement>(".project-img img")]
    .map((image) => image.getAttribute("src"))
    .filter((source): source is string => typeof source === "string" && source.startsWith("/") && !source.startsWith("//") && !source.includes("\\")));
}

function canStartProjectEffect(windowRef: ProjectWindow, documentRef: Document) {
...
function announceProjectStart(windowRef: ProjectWindow, images: readonly string[], outcome: ProjectOutcome) {
  const detail: ProjectStartDetail = Object.freeze({ count: images.length, images, ...outcome });
  windowRef.dispatchEvent(new windowRef.CustomEvent(PROJECT_START_EVENT, { detail, cancelable: false }));
}

function attemptProjectStart(windowRef: ProjectWindow, documentRef: Document): ProjectOutcome {
...
export function startProjectEffect({ windowRef = globalThis.window, documentRef = globalThis.document }: { windowRef?: ProjectWindow; documentRef?: Document } = {}) {
