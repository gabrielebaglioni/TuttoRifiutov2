...
interface Particle { ox: number; oy: number; vx: number; vy: number; i: number; j: number }
interface ParticleState {
 config: { logoSize: number; distortionRadius: number; forceStrength: number; maxDisplacement: number; returnForce: number; logoPath: string; particleSpacing: number };
 canvas: HTMLCanvasElement | null; gl: WebGLRenderingContext | null; program: WebGLProgram | null;
 geometry: { posBuf: WebGLBuffer | null; colBuf: WebGLBuffer | null; count: number } | null;
 particles: Particle[]; posArray: Float32Array<ArrayBuffer> | null; colorArray: Float32Array<ArrayBuffer> | null;
 mouse: { x: number; y: number }; execCount: number; isMobile: boolean;
 animFrame: number; isAnimating: boolean; isVisible: boolean; failed: boolean;
 touch: ReturnType<typeof installTouchExplosion> | null; touchStrength: number;
 touchPoint: { x: number; y: number }; bounds: { left: number; top: number; right: number; bottom: number } | null;
 rasterSize: number;
}
const PV: ParticleState = {
  config: {
    logoSize: 3000,
...
function init() {
  PV.canvas = document.querySelector<HTMLCanvasElement>("#particle-canvas");
  if (!PV.canvas) return;
  ...
  const visibilityObserver = new IntersectionObserver(([entry]) => {
    PV.isVisible = entry?.isIntersecting ?? false;
  ...
  try { PV.gl = PV.canvas.getContext("webgl", contextOptions); } catch { showParticleFallback(); return; }
  ...
  const unbindTheme = publicTheme().subscribe(({rgb}) => {
    if (PV.failed || !PV.gl || !PV.program) return;
  ...
  if (PV.isMobile && PV.particles.length) {
    ...
    PV.touch = installTouchExplosion({
      ...
      enabled:()=>PV.isMobile && !PV.failed && PV.isVisible && !prefersReducedMotion() &&
        !document.querySelector('.menu-toggle-btn[aria-expanded="true"]') && (PV.canvas?.getBoundingClientRect().top ?? -Infinity) > -24,
...
function animate() {
  ...
  if (!PV.gl || !PV.geometry || !PV.posArray) return;
  ...
  if (!PV.isMobile && PV.execCount > 0) {
    ...
    for (let i = 0; i < PV.particles.length; i++) {
      ...
      if (!p || x === undefined || y === undefined) continue;
...
// webgl render
function render() {
  if (PV.failed || !PV.canvas || !PV.gl || !PV.program || !PV.geometry) return;
...
function handleResize() {
  if (PV.failed || !PV.canvas || !PV.gl || !PV.posArray) return;
