...
interface PieState {
 svg: SVGSVGElement | null; container: HTMLElement | null;
 pieGroup: SVGGElement | null; dotsGroup: SVGGElement | null;
 headerSplit: SplitText | null; scrollTrigger: ScrollTrigger | null;
 canvas: ReturnType<typeof createPieCanvas>; visibleWords: number;
}
...
function init() {
  STATE.container = document.querySelector<HTMLElement>(".pie-transition");
  ...
  Promise.all([document.fonts.ready, contentReady]).then(() => {
    setupHeader();
    renderProgress(STATE.scrollTrigger?.progress ?? 0);
    ScrollTrigger.sort();
...
function appendMaskDefs() {
  ...
  if ("decode" in logoMaskImage && typeof logoMaskImage.decode === "function") {
    Promise.resolve(logoMaskImage.decode()).catch(() => {});
  }
}
...
function setupScrollTrigger() {
  ...
  if (mobile && !parent) return;
  STATE.scrollTrigger = ScrollTrigger.create({
    trigger: mobile ? parent ?? container : container,
    start: "top top",
    end: () => `+=${mobile ? (parent?.offsetHeight ?? container.offsetHeight) - container.offsetHeight : container.offsetHeight * PIN_LENGTH_VIEWPORTS}`,
    scrub: true,
    pin: !mobile,
    anticipatePin: 1,
    pinSpacing: true,
...
function renderProgress(progress: number) {
      ...
      else if (STATE.pieGroup && STATE.dotsGroup) {
        updatePieFill(fill);
