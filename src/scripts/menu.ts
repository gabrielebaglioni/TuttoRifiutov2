...
let menuItems = SITE_CONTENT["global.menu.items"].map(([label = "", href = ""], index) => ({
  label,
  icon: MENU_ICONS[index % MENU_ICONS.length] ?? "cube-sharp",
  href,
}));

let isOpen = false;
let isMenuAnimating = false;
let responsiveConfig: ReturnType<typeof getResponsiveConfig> = { menuSize: 0, center: 0, innerRadius: 0, outerRadius: 0, contentRadius: 0 };
let resetJoystick: (() => void) | null = null;
let menuLinkSplits: SplitText[] = [];
let menuLinkAnimations: gsap.core.Tween[] = [];
let returnFocus: HTMLElement | SVGElement | null = null;
let backgroundAccess: Array<[HTMLElement, boolean]> = [];
let previousOverflow = '';
let resumeScroll = false;

let atmosphereScene: ThreeTypes.Scene | null = null;
let atmosphereCamera: ThreeTypes.OrthographicCamera | null = null;
let atmosphereRenderer: ThreeTypes.WebGLRenderer | null = null;
let atmosphereMaterial: ThreeTypes.ShaderMaterial;
let atmosphereMesh: ThreeTypes.Mesh;
let atmosphereUniforms: { iTime: ThreeTypes.IUniform<number>; iResolution: ThreeTypes.IUniform<ThreeTypes.Vector2> } | null = null;
let lastAtmosphereFrame: number | null = null;
let atmosphereAttempted = false;
let atmosphereFailed = false;
let atmosphereFrame = 0;
let THREE: Awaited<ReturnType<typeof loadMenuLibrary>> | null = null;

// initialization
document.addEventListener("DOMContentLoaded", () => {
  responsiveConfig = getResponsiveConfig();

  const menu = document.querySelector<HTMLElement>(".circular-menu");
  const joystick = document.querySelector<HTMLElement>(".joystick");
  const menuOverlayNav = document.querySelector<HTMLElement>(".menu-overlay-nav");
  const menuOverlayFooter = document.querySelector<HTMLElement>(".menu-overlay-footer");
  ...
  document
    .querySelector<HTMLElement>(".menu-toggle-btn")
    ?.addEventListener("click", toggleMenu);
  document.querySelector<HTMLElement>(".close-btn")?.addEventListener("click", toggleMenu);
...
function renderSegments(menu: HTMLElement) {
  menu.querySelectorAll<HTMLElement>(".menu-segment").forEach((segment) => segment.remove());
...
document.addEventListener("tutto-rifiuto:content", (event) => {
  ...
  menuItems = rows.map(([label = "", href = ""], index) => ({ label, href, icon: MENU_ICONS[index % MENU_ICONS.length] ?? "cube-sharp" }));
  const menu = document.querySelector<HTMLElement>(".circular-menu");
...
function initMenuLinkFlicker(link: Element) {
...
function getResponsiveConfig() {
  ...
  const viewportHeight = document.querySelector<HTMLElement>('.menu-overlay')?.clientHeight || window.innerHeight;
...
function showAtmosphereFallback() {
  ...
  document.querySelector<HTMLElement>('.menu-overlay')?.classList.add('has-atmosphere-fallback');
  const canvas = document.getElementById('menu-canvas');
  if (canvas) canvas.style.display = 'none';
}

function initAtmosphere() {
  const canvas = document.querySelector<HTMLCanvasElement>("#menu-canvas");
  ...
  atmosphereUniforms = { iTime: { value: 0 }, iResolution: { value: new THREE.Vector2() } };
  ...
  const unbindTheme = bindThemeUniforms(atmosphereMaterial.uniforms, {uColorBg:'accent', uColorFg:'foreground'}, () => {
    if (!atmosphereFailed && atmosphereRenderer && atmosphereScene && atmosphereCamera) atmosphereRenderer.render(atmosphereScene, atmosphereCamera);
  });
...
function resizeAtmosphere() {
  if (!atmosphereRenderer || atmosphereFailed || !atmosphereUniforms) return;
  ...
  atmosphereUniforms.iResolution.value.set(width, height);
}

function animateAtmosphere(time = 0) {
  if (atmosphereFailed || !atmosphereUniforms || !atmosphereRenderer || !atmosphereScene || !atmosphereCamera) return;
  ...
  atmosphereUniforms.iTime.value += touch ? Math.min(elapsed, 100) / 1000 : 0.016;
...
function setMenuAccess(open: boolean) {
  const overlay = document.querySelector<HTMLElement>('.menu-overlay');
  const toggle = document.querySelector<HTMLElement>('.menu-toggle-btn');
  ...
  if (open) {
    ...
    backgroundAccess = [...document.body.children].filter((node): node is HTMLElement => node instanceof HTMLElement).filter(node => node !== overlay && !node.contains(overlay)).map(node => [node, Boolean(node.inert)]);
    ...
    document.querySelector<HTMLElement>('.close-btn')?.focus({ preventScroll: true });
  } else {
...
function handleMenuKeydown(event: KeyboardEvent) {
  ...
  const links = [...(document.querySelector<HTMLElement>('.menu-overlay')?.querySelectorAll<HTMLElement>('button, a[href]') ?? [])];
  const first = links[0], last = links[links.length - 1];
  if ((event.shiftKey && document.activeElement === first) || (!event.shiftKey && document.activeElement === last) || !links.some(link => link === document.activeElement)) {
    event.preventDefault();
...
function createSegment(item: { label: string; href: string; icon: string }, index: number, total: number) {
...
function updateSegment(segment: HTMLElement, index: number, total: number) {
  const { menuSize, pathData, contentX, contentY } = calculateSegmentGeometry(
    index,
    total,
  );
  ...
  const segmentContent = segment.querySelector<HTMLElement>(".segment-content");
...
function resizeMenu() {
  ...
  const menuSegments = document.querySelectorAll<HTMLElement>(".menu-segment");
...
function toggleMenu() {
  ...
  if (!isOpen) {
    ...
    const menuFooterLinks = menuOverlayFooter?.querySelectorAll<HTMLElement>("a");
  ...
  function animate() {
    ...
    if (
      ...
      const segmentIndex =
        ...
        menuItems.length;
      const segment = document.querySelectorAll<HTMLElement>(".menu-segment")[segmentIndex];

      if (segment && segment !== activeSegment) {
        if (activeSegment) {
          activeSegment.style.animation = "";
          const content = activeSegment.querySelector<HTMLElement>(".segment-content");
